# Hotel Booking Demand Analysis (SQL Data Analysis Project)

## Project Overview
This project analyzes hotel booking demand patterns using a dataset of over 119,000 hotel reservations.  
The analysis explores tourism demand trends, seasonal booking behavior, cancellation patterns, and revenue drivers.

---

## Tools Used
- SQL
- DBeaver
- Google Sheets
- GitHub

---

## Dataset
Hotel Booking Demand Dataset  
Total Records: 119,390  
Variables: 32 columns

---

## Key Business Questions
1. Which countries generate the most hotel bookings?
2. Which months have the highest hotel demand?
3. Which hotel type generates the most revenue?
4. What is the hotel booking cancellation rate?
5. Which countries generate the highest revenue?

---

## SQL Queries

The SQL queries used for this analysis can be found here:

[View SQL Queries](hotel_booking_analysis.sql)

---

## Key Insights
- Portugal generates the highest number of bookings and revenue as the dataset represents hotels located in Portugal.
- International demand is driven mainly by travelers from the United Kingdom, France, Spain, and Germany.
- Hotel bookings peak during summer months, especially July and August.
- City Hotels contribute nearly 70% of total revenue compared to Resort Hotels.
- Approximately 37% of bookings are cancelled, indicating a significant cancellation rate in the hotel industry.

---

## Visualizations

### Top Countries by Hotel Bookings
![Top Countries](charts/chart1_top_countries.png)

### Monthly Booking Trends
![Monthly Trends](charts/chart2_monthly_trends.png)

### Revenue by Hotel Type
![Revenue Hotel Type](charts/chart3_revenue_hotel_type.png)

### Booking Cancellation Rate
![Cancellation Rate](charts/chart4_cancellation_rate.png)

### Revenue by Country
![Revenue by Country](charts/chart5_revenue_by_country.png)

---

## Author
Gowrish Nambiar  
Aspiring Data Analyst | SQL | Data Analytics
